...
const transporter = nodemailer.createTransport({
  host: process.env.SMTP_HOST || 'mail.ferozo.com',
...
app.post('/enviar-cita', async (req, res) => {
  ...
  const htmlCorreo = `
    <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;border:1px solid #dde3ed;border-radius:12px;overflow:hidden;">
      <div style="background:#1C2D4A;padding:20px 24px;display:flex;align-items:center;gap:12px;">
        <div>
          <h1 style="color:#ffffff;font-size:18px;margin:0;">ID Lillo SpA</h1>
          <p style="color:#C5980A;font-size:11px;margin:0;text-transform:uppercase;letter-spacing:1px;">Ingeniería Eléctrica — Nueva Solicitud de Cita</p>
        </div>
      </div>
